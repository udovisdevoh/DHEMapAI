...
namespace DGraphBuilder.Generation
{
    public class MapBuilder
    {
        ...
        public DhemapFile Build(List<PhysicalRoom> layout)
        {
            ...
            // CORRECTION : On ne se contente plus de connecter les pièces les plus proches.
            // On s'assure que les connexions suivent un chemin logique sans croisement.
            var connectionGraph = new Dictionary<string, List<string>>();
            foreach (var room in _dgraph.Rooms) { connectionGraph[room.Id] = new List<string>(); }
            foreach (var c in _dgraph.Connections) { connectionGraph[c.FromRoom].Add(c.ToRoom); connectionGraph[c.ToRoom].Add(c.FromRoom); }
            ...
            foreach (var connection in _dgraph.Connections)
            {
                ...
                {
                    // On ne connecte que les pièces physiquement adjacentes.
                    var (fromWall, toWall) = FindClosestWalls(fromRoom, toRoom);
                    if (fromWall != null && toWall != null)
                    {
                        float dist = Vector2.Distance(GetLinedefMidpoint(fromWall), GetLinedefMidpoint(toWall));
                        // Seuil de distance pour considérer deux pièces comme adjacentes
                        if (dist < (fromRoom.PlacedRoom.Size.X + toRoom.PlacedRoom.Size.X))
                        {
        ...
        private void CarveConnection(BuiltRoom from, BuiltRoom to, Connection connection)
        {
            ...
            var doorSector = new Sector
            {
                ...
                CeilingHeight = from.DhemapSector.FloorHeight,
        ...
        private (Linedef opening, Linedef[] newWalls) SplitLinedef(BuiltRoom room, Linedef line, float openingSize)
        {
            ...
            if (openingSize >= lineLength - 8) openingSize = lineLength - 8;
        ...
        private (Linedef, Linedef) FindClosestWalls(BuiltRoom r1, BuiltRoom r2) { /* ... */ Linedef bestL1 = null, bestL2 = null; float minDist = float.MaxValue; foreach (var l1 in r1.Linedefs.Where(l => l.BackSidedef == null)) { foreach (var l2 in r2.Linedefs.Where(l => l.BackSidedef == null)) { var mid1 = GetLinedefMidpoint(l1); var mid2 = GetLinedefMidpoint(l2); float dist = Vector2.DistanceSquared(mid1, mid2); if (dist < minDist) { minDist = dist; bestL1 = l1; bestL2 = l2; } } } return (bestL1, bestL2); }
