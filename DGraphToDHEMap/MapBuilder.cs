    ...
    public class MapBuilder
    {
        ...
        private void CarveConnection(BuiltRoom from, BuiltRoom to, Connection connection, Dictionary<string, BuiltRoom> allRooms)
        {
            ...
            var doorSector = new Sector
            {
                ...
                CeilingHeight = from.DhemapSector.FloorHeight, // Commence fermé
        ...
        {
            Func<Vector2, Vector2, Vector2, Vector2, bool> linesIntersect = (a, b, c, d) =>
            {
            ...
            var btmLeft = new Vector2(rect.Left, rect.Bottom);
            var btmRight = new Vector2(rect.Right, rect.Bottom);

            if (linesIntersect(p1, p2, topLeft, topRight)) return true;
            if (linesIntersect(p1, p2, topRight, btmRight)) return true;
            if (linesIntersect(p1, p2, btmRight, btmLeft)) return true;
            if (linesIntersect(p1, p2, btmLeft, topLeft)) return true;
        ...
        private BuiltRoom BuildRoomGeometry(PhysicalRoom pRoom) { var rect = pRoom.GetBoundingBox(); var sector = CreateSectorFromRoom(pRoom.DGraphRoom); _dhemap.Sectors.Add(sector); var v1 = AddVertex(rect.Left, rect.Top); var v2 = AddVertex(rect.Right, rect.Top); var v3 = AddVertex(rect.Right, rect.Bottom); var v4 = AddVertex(rect.Left, rect.Bottom); string wallTexture = GetTexture(pRoom.DGraphRoom.Properties.WallTexture); var builtRoom = new BuiltRoom { PlacedRoom = pRoom, DhemapSector = sector }; builtRoom.Linedefs.Add(AddLinedef(v1, v2, sector.Id, wallTexture)); builtRoom.Linedefs.Add(AddLinedef(v2, v3, sector.Id, wallTexture)); builtRoom.Linedefs.Add(AddLinedef(v3, v4, sector.Id, wallTexture)); builtRoom.Linedefs.Add(AddLinedef(v4, v1, sector.Id, wallTexture)); PlaceThings(pRoom.DGraphRoom, rect); return builtRoom; }
