...
### 2.4. L'objet `themePalette` (Optionnel)
Cet objet, s'il est présent, définit une palette de concepts qui peuvent être référencés par les `thematicTokens`. Cela permet de séparer la définition d'un thème visuel (les textures spécifiques) de la logique structurelle de la carte.
...
C'est le cœur du format. Il définit l'ensemble des "pinceaux conceptuels" que le générateur peut utiliser, ainsi que leurs interactions. Chaque objet du tableau est un **Token**.

#### L'objet Token
| Clé              | Type               | Description                                                                                                                                                             |
|------------------|--------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `name`           | Chaîne             | Le nom descriptif et lisible du **concept thématique** (ex: "Murs de la base", "Sol de lave"). Sert d'identifiant unique pour les `adjacencyRules`.                     |
| `type`           | Chaîne             | Le type de token. **Enum**: `"wall"`, `"flat"`, `"object"`, `"connection_action"`.                                                                                    |
| `paletteConcept` | Chaîne \| `null`   | Si le `type` est `"wall"`, `"flat"`, ou `"connection_action"`, ceci est la clé correspondante dans la `themePalette` pour choisir l'apparence. Sinon, `null`.          |
| `typeId`         | Entier \| `null`   | Si le `type` est `"object"`, ceci est l'identifiant numérique canonique du 'thing' à placer. C'est cette valeur qui doit être utilisée par le générateur. Sinon, `null`. |
...
| `baseWeight`     | Entier             | La probabilité de base (non-contextuelle) de ce token d'être choisi parmi tous les tokens du même `type`.                                                              |
| `adjacencyRules` | Tableau d'objets   | Un tableau de règles qui modifient le poids du token en fonction de ses voisins. Voir la section 2.5.1.                                                                |
...
| `adjacentTo`       | Chaîne | Cible un autre token par son `name`. Utile pour lier des concepts architecturaux (ex: un mur de support à côté d'un mur principal).                 |
| `adjacentToTypeId` | Entier | Cible un token de type `object` par son `typeId`. Méthode robuste pour cibler des 'things' spécifiques (ex: un Démon apparaît plus souvent près d'un Diablotin). |

---

## 3. Fichier d'Exemple Canonique

Voici un exemple complet et valide qui illustre comment ces concepts s'assemblent pour définir un thème "Forge Industrielle Infernale", en séparant clairement la palette visuelle de la logique de placement.
